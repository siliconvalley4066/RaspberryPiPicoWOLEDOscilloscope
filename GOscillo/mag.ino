/*
 * sin(x)/x interpolation for timebase magnification at x5 and x10.
  ...
  for (int i = 0; i < SAMPLES; i++) {
    ...
    if (m == 0) {
      ...
      for (n = 0; n < MAGSAMPL; n++) {
        sum += d[s - n] * sinc40[m + m + 10 * n];
      }
      for (n = 1; n < MAGFORWD; n++) {
        sum += d[s + n] * sinc40[10 * n - m - m];
      }
      sum /= 4096;
    }
